...
export function RegistrationForm() {
  ...
  const [selected, setSelected] = useState<string>(pujaOptions[0][0]);
  const selectedInfo = useMemo(() => pujaOptions.find((item) => item[0] === selected)?.[1], [selected]);
  ...
  return (
    <form onSubmit={submit} noValidate>
      ...
      <label>報名項目＊<select name="service" required value={selected} onChange={(event) => setSelected(event.target.value)}>{pujaOptions.map((item) => <option key={item[0]}>{item[0]}</option>)}</select><small>{selectedInfo}</small></label>
